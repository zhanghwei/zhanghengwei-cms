...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
	  ...
	  <div class="form-group mx-sm-3 mb-2">
	    <input type="text" value="${article.title}" name="title" class="form-control" placeholder="请输入文章标题">
	  </div>
	  <div class="form-group mx-sm-3 mb-2">
  ...
  <tbody>
	<c:forEach items="${pageInfo.list }" var="item">
       <tr>
	     ...
	     <td>${item.status==1?"已审核":item.status==0?"未审核":item.status==-1?"审核未通过":item.status==3?"已被禁用":"草稿"}</td>
	      ...
	      <td>
	      	...
	      	<button type="button" class="btn btn-primary" onclick="jinkan('${item.id}')">禁止查看</button>
	      </td>
	    </tr>
   	</c:forEach>
  </tbody>
